<form action="{{ route('CmsListAuditorEnergyStoreData') }}" method="POST" id="ListAuditorEnergyManagementFrom" class="form" enctype="multipart/form-data" @submit.prevent>
				...
				<div class="create__form content__tab" v-for="(supportedLang, supportedLangKey, index) in supported_language" :class="!index ? 'active__content' : ''" :id="'lang-'+supportedLangKey">
					<div class="form__group__row">
						...
						<div :id="'form-accordion-'+supportedLangKey+'-1'">
							...
							<div class="create__form__row" v-if="showElementByDefaultLang(supportedLangKey)">
								<div class="new__form__field">
									<label>Type Auditor</label>
									
									<select name="type_auditor" v-model="models.type_auditor" id="type_auditor">
										<option value="">Select Type Auditor</option>
										<option value="internal">Internal</option>
										<option value="eksternal">Eksternal</option>
									</select>

									<div class="form--error--message--left" id="form--error--message--type_auditor"></div>
								</div>
							</div>

							<div class="create__form__row" v-if="showElementByDefaultLang(supportedLangKey)">
								<div class="new__form__field">
									<label>Certified Years</label>
									
									<select name="years" v-model="models.years" id="years">
										<option value="">Select Years</option>
										<option :value="year" v-for="year in available_year">@{{ year }}</option>
									</select>

									<div class="form--error--message--left" id="form--error--message--years"></div>
								</div>
							</div>
